...
/**
  ...
  */
/**
  * @brief          哨兵云台陀螺仪控制，电机是增量角度控制，
  * @param[in]      yaw: yaw轴角度控制，为角度的增量 单位 rad
  * @param[in]      pitch: pitch轴角度控制，为角度的增量 单位 rad
  * @param[in]      gimbal_control_set: 云台数据指针
  * @retval         none
  */
static void gimbal_add_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set);
...
//云台行为状态机
static gimbal_behaviour_e gimbal_behaviour = GIMBAL_OFF;
...
{   
    ...
    //根据云台行为状态机设置电机状态机
    if (gimbal_behaviour == GIMBAL_OFF)
    {
			gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_OFF;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_OFF;
    }
    else if (gimbal_behaviour == GIMBAL_REMOTE)
    { 
			gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_REMOTE;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_REMOTE;
    }
    else if (gimbal_behaviour ==GIMBAL_VISION )
    {
        gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_IMU;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_IMU;
    }
...
void gimbal_behaviour_control_set(fp32 *add_yaw, fp32 *add_pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    {
        gimbal_add_angle_control(add_yaw, add_pitch, gimbal_control_set);
		}
		else
		{
...
{ 
    ...
    {
       gimbal_behaviour = GIMBAL_OFF;
    }
    else if (switch_is_mid(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]))
    {
        gimbal_behaviour = GIMBAL_REMOTE ;
    }
    if(IF_vision_Start == true)
		{
			  gimbal_behaviour = GIMBAL_VISION;
		}
...
static void gimbal_cali_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    if (gimbal_control_set->gimbal_cali.step == GIMBAL_CALI_PITCH_MAX_STEP)
    {
        ...
        *pitch = 0.1*GIMBAL_CALI_PTICH_MOTOR_SET;//向下惯性较大
        *yaw = 0;
    ...
    {
        *pitch = -GIMBAL_CALI_PTICH_MOTOR_SET; 
        *yaw = 0;
...
/**
  ...
  */
/**
  * @brief          哨兵云台陀螺仪控制，电机是陀螺仪角度控制，
  * @param[out]     yaw: yaw轴角度控制，为角度的设定 单位 rad
  * @param[out]     pitch:pitch轴角度控制，为角度的设定 单位 rad
  * @param[in]      gimbal_control_set:云台数据指针
  * @retval         none
  */
static void gimbal_absolute_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    // *yaw = yaw_channel * PI / 660.0;
    // *pitch = pitch_channel * PITCH_RC_SEN + gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN;
    
    fp32 dyaw, dpitch;
    fp32 yaw_temp,pitch_temp;
    AutoAimConsole(&yaw_temp, &pitch_temp);
    dyaw = K_ADD_DYAW * (yaw_temp - get_INS_angle_point()[0]);//给增量提高响应速度
    dyaw = fp32_constrain(dyaw, -ADD_MAX_DYAW, ADD_MAX_DYAW);
    *pitch = pitch_temp;
    *yaw = yaw_temp ;
