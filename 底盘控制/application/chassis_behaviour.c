...
/**
  * @brief          自动模式，底盘模式是不跟随角度，底盘旋转速度由参数直接设定
  * @author         RM
  * @param[in]      vx_set前进的速度,正值 前进速度， 负值 后退速度
  * @param[in]      vy_set左右的速度,正值 左移速度， 负值 右移速度
  * @param[in]      wz_set底盘设置的旋转速度,正值 逆时针旋转，负值 顺时针旋转
...
static void chassis_aoto_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector);
...
//留意，这个底盘行为模式变量
chassis_behaviour_e chassis_behaviour_mode = CHASSIS_AOTO;
...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    {
        chassis_behaviour_mode = CHASSIS_AOTO;
    ...
    else if (chassis_behaviour_mode == CHASSIS_OPEN)
    {
		...
		else if (chassis_behaviour_mode == CHASSIS_AOTO)  // 自动模式
    {
...
void chassis_behaviour_control_set(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    else if (chassis_behaviour_mode == CHASSIS_OPEN)
    {
		...
		else if (chassis_behaviour_mode == CHASSIS_AOTO)
    {
        chassis_aoto_control(vx_set, vy_set, angle_set, chassis_move_rc_to_vector);
    }
}
...
{   extern gimbal_control_t gimbal_control;
	  gimbal_control.gimbal_yaw_motor.max_relative_angle =  0.1f;//max_yaw;
    gimbal_control.gimbal_yaw_motor.min_relative_angle =  -0.1f;//min_yaw;
    if (vx_set == NULL || vy_set == NULL || wz_set == NULL || chassis_move_rc_to_vector == NULL)
    {
        return;
    }
    ...
    chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);
    *wz_set = -CHASSIS_WZ_RC_SEN * chassis_move_rc_to_vector->chassis_RC->rc.ch[CHASSIS_WZ_CHANNEL]- chassis_move_rc_to_vector->chassis_RC->mouse.x * 0.3f;
}
