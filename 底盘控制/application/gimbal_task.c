...
/**
  ...
  */
/**
  * @brief          哨兵云台控制模式:GIMBAL_MOTOR_ENCONDE，使用陀螺仪增量进行控制
  * @param[out]     gimbal_motor:yaw电机或者pitch电机
  * @retval         none
  */
static void gimbal_motor_add_angle_control(gimbal_motor_t *gimbal_motor);
/**
...
static void gimbal_add_pitch_angle_limit(gimbal_motor_t *gimbal_motor, fp32 add);
static void gimbal_add_yaw_angle_limit(gimbal_motor_t *gimbal_motor, fp32 add);
/**
...
void gimbal_task(void const *pvParameters)
{
    ...
    while (1)
    {
        ...
        shoot_control_loop(&shoot_current);        //射击任务控制循环
        ...
        shoot_can_set_current = shoot_current.trigger_give_current;
        if(!(toe_is_error(YAW_GIMBAL_MOTOR_TOE) && toe_is_error(PITCH_GIMBAL_MOTOR_TOE) && toe_is_error(TRIGGER_MOTOR_TOE)))
        {
            ...
            { 
              ...
              CAN_cmd_gimbal(0, 0, 0);
                ...
                OutputPCData.data_1 = gimbal_control.gimbal_yaw_motor.gimbal_motor_measure->ecd;
            ...
            { 
              ...
              CAN_cmd_fric(0,0,0);
						...
						else
						{
						  ...
						  CAN_cmd_fric(shoot_current.fric_give_current[0],
                           shoot_current.fric_give_current[1],
														shoot_current.trigger_give_current);
...
static void gimbal_init(gimbal_control_t *init)
{
    ...
    init->gimbal_rc_ctrl = get_remote_control_point();
		//初始化pitch限位
		init->limit_pitch_min = -24/RadiansToDegrees;
		init->limt_pitch_max = 25/RadiansToDegrees;
    //初始化电机模式
    init->gimbal_yaw_motor.gimbal_motor_mode = init->gimbal_yaw_motor.last_gimbal_motor_mode = GIMBAL_MOTOR_IMU;
    init->gimbal_pitch_motor.gimbal_motor_mode = init->gimbal_pitch_motor.last_gimbal_motor_mode = GIMBAL_MOTOR_IMU;
    //初始化yaw电机pid
...
static void gimbal_mode_change_control_transit(gimbal_control_t *gimbal_mode_change)
{
    ...
    {
        gimbal_mode_change->gimbal_yaw_motor.relative_angle_set = gimbal_mode_change->gimbal_yaw_motor.absolute_angle;//实则为陀螺仪增量控制
    }
    ...
    {
        gimbal_mode_change->gimbal_pitch_motor.relative_angle_set = gimbal_mode_change->gimbal_pitch_motor.absolute_angle;//实则为陀螺仪增量控制
    }
...
static void gimbal_set_control(gimbal_control_t *set_control)
{
    ...
    fp32 yaw_angle = 0.0f;
    fp32 pitch_angle = 0.0f;

    gimbal_behaviour_control_set(&add_yaw_angle, &add_pitch_angle, set_control);//获取角度增量
    gimbal_behaviour_control_set(&yaw_angle, &pitch_angle, set_control);//获取设置的角度

    //yaw电机模式控制
    if (set_control->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_OFF)
    {
        //raw模式下，直接发送控制值
        set_control->gimbal_yaw_motor.absolute_angle_set = yaw_angle;
    }
    else if (set_control->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_IMU)
    {
        //gyro模式下，陀螺仪角度进行位置控制
        gimbal_absolute_yaw_angle_limit(&set_control->gimbal_yaw_motor, yaw_angle);
    }
    else if (set_control->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_REMOTE)
    {
        //enconde模式下，电机陀螺仪增量角度控制
        gimbal_add_yaw_angle_limit(&set_control->gimbal_yaw_motor, add_yaw_angle);
    }
		else
		{
    ...
    //pitch电机模式控制
    if (set_control->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_OFF)
    {
        //raw模式下，直接发送控制值
        set_control->gimbal_pitch_motor.absolute_angle_set = pitch_angle;
    }
    else if (set_control->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_IMU)
    {
        //gyro模式下，陀螺仪角度控制
        gimbal_absolute_pitch_angle_limit(&set_control->gimbal_pitch_motor, pitch_angle);
    }
    else if (set_control->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_REMOTE)
    {
        //enconde模式下，电机陀螺仪增量角度控制
        gimbal_add_pitch_angle_limit(&set_control->gimbal_pitch_motor, add_pitch_angle);
    }
		else
		{
...
/**
  ...
  */
/**
  * @brief          哨兵云台控制模式:GIMBAL_MOTOR_GYRO，使用陀螺仪计算的欧拉角进行控制
  * @param[out]     gimbal_motor:pitch电机
  * @retval         none
  */
static void gimbal_absolute_pitch_angle_limit(gimbal_motor_t *gimbal_motor, fp32 angle_set)
{
    ...
    gimbal_motor->absolute_angle_set = rad_format(angle_set);
		VAL_LIMIT(gimbal_motor->absolute_angle_set,-24/RadiansToDegrees,25/RadiansToDegrees);
}
...
/**
  ...
  * @param[in]      angle_set:角度设定值
  * @retval         none
  * @note           位置控制
  */
static void gimbal_absolute_yaw_angle_limit(gimbal_motor_t *gimbal_motor, fp32 angle_set)
{
    if (gimbal_motor == NULL)
    {
        return;
...
static void gimbal_control_loop(gimbal_control_t *control_loop)
{
    ...
    else if (control_loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_IMU)
    {
        gimbal_motor_absolute_angle_control(&control_loop->gimbal_yaw_motor);
    }
    else if (control_loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_REMOTE)
    {
        gimbal_motor_add_angle_control(&control_loop->gimbal_yaw_motor);
    }
		else
		{
    ...
    else if (control_loop->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_IMU)
    {
        gimbal_motor_absolute_angle_control(&control_loop->gimbal_pitch_motor);
    }
    else if (control_loop->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_REMOTE)
    {
        gimbal_motor_add_angle_control(&control_loop->gimbal_pitch_motor);
    }
		else
		{
